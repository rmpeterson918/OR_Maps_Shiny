...
#       
#       geom_sf_text(data = filtered_geo(), 
#                    aes(label = name), 
#                    size = 2.5, 
#                    color = "darkblue", 
#                    fontface = "bold"
  ...
  tabsetPanel(
    ...
    tabPanel("Interactive AVA Map",
             sidebarLayout(
               ...
               mainPanel(
                 width = 9,
                 plotlyOutput("avaMapInteractive", height = "700px")
               )
             )
    )
  )
...
server <- function(input, output, session) {
  ...
  # 1. REACTIVE DATA FILTER (For Tab 1)
  filtered_geo <- reactive({
    if (input$selected_ava == "All") {
      geo_data
    } else {
  ...
  # 2. TAB 1: INTERACTIVE MAP (Plotly)
  output$avaMapInteractive <- renderPlotly({
    my_map <- ggplot() +
      geom_sf(data = or_counties, fill = "grey95", color = "#999999") +
      geom_sf_text(data = or_counties, 
                   aes(label = NAME),  
                   size = 2.5, 
                   color = "#999999", 
                   ...
                   nudge_x = 0.05,
                   nudge_y = 0.05) +
      geom_sf(data = filtered_geo(), fill = "#4393c3", color = "blue", size = 0.2, alpha = 0.6) +
      geom_sf_text(data = filtered_geo(), 
                   aes(label = name), 
                   size = 2.5, 
                   color = "darkblue", 
  ...
  }, res = 96) # Higher resolution rendering for crisp text
} 
